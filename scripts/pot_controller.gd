...
var _watering_loop_playing: bool = false
...
func _process(delta: float) -> void:
	if _is_following_mouse:
		...
		if _left_pressed:
			...
			if not _watering_loop_playing and _left_press_time >= 0.5:
				AudioManager.play_sfx_loop("watering")
				_watering_loop_playing = true
	else:
...
func _unhandled_input(event: InputEvent) -> void:
	...
	# 当水壶跟随鼠标时，处理左键点击/长按的音效逻辑
	if _is_following_mouse and mouse_event.button_index == MOUSE_BUTTON_LEFT:
		if mouse_event.pressed:
			_left_pressed = true
			_left_press_time = 0.0
			_watering_loop_playing = false
		else:
			# 松开时根据是否已经进入循环决定：单击音效 或 停止循环音效
			if _left_pressed:
				if _watering_loop_playing:
					AudioManager.stop_all_sfx("watering")
				else:
