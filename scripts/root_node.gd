...
func _ready() -> void:
	...
	var aspect_ratio : int # 用来表示窗口宽高比的内部变量
	...
	DisplayServer.window_set_flag(DisplayServer.WINDOW_FLAG_BORDERLESS, true) #把【窗口栏】变成透明的。现在窗口是无边框全透明了。
	...
	window_position = Vector2i(0,screen_height - screen_width/aspect_ratio) # 表示窗口位置的变量，横坐标是屏幕最左边，纵坐标是屏幕高度-窗口高度
	DisplayServer.window_set_position(window_position) # 设置游戏窗口位置（显示在屏幕底端）
	
	
	# 用来设置【左】菜单的：位置、大小、缩放。不要直接设置固定数值，不然电脑分辨率调整后游戏比例会失调
	menu_left = $menu_left # 菜单窗口
	#menu_left.size = Vector2(screen_width/2,viewport_height) #左菜单大小
	...
	# 以下用来调试
	print("screen width变量表达的屏幕可用宽度: ", DisplayServer.screen_get_usable_rect().size.x)
	print("屏幕全部宽度: ", DisplayServer.screen_get_size().x)
	print("窗口宽度: ", get_window().size.x)
